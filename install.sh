#!/usr/bin/env bash
#
# install.sh — install Tally (the menu bar app + tally-cli) on macOS.
#
# What it does, in order — nothing hidden:
#   1. Refuses to run anywhere but macOS 14+ on Apple Silicon.
#   2. Downloads Tally.app.zip + tally-cli from the *pinned* GitHub Release
...
[ "$(uname -s)" = "Darwin" ] || die "Tally is macOS-only (this is $(uname -s))."
[ "$(uname -m)" = "arm64" ]  || die "Tally needs Apple Silicon (this Mac reports $(uname -m)); no Intel build is shipped."
OS_MAJOR="$(sw_vers -productVersion | cut -d. -f1)"
[ "${OS_MAJOR:-0}" -ge 14 ]  || die "Tally needs macOS 14 or newer (you're on $(sw_vers -productVersion))."
...
say "Downloading Tally $TAG from github.com/$REPO …"
fetch "SHASUMS256.txt"
fetch "Tally.app.zip"
fetch "tally-cli"
ok "downloaded Tally.app.zip + tally-cli (+ SHASUMS256.txt)"

say "Verifying SHA-256 against SHASUMS256.txt…"
verify "Tally.app.zip" "tally-cli"
...
mkdir -p "$APP_DIR"
rm -rf "$APP"
ditto -x -k "$TMP/Tally.app.zip" "$APP_DIR"
...
mkdir -p "$BIN_DIR"
cp -f "$TMP/tally-cli" "$BIN_DIR/tally-cli"
chmod +x "$BIN_DIR/tally-cli"
ok "installed (the widget finds it here even if ~/.local/bin isn't on your PATH)"
case ":${PATH}:" in
  *":$BIN_DIR:"*) : ;;
  *) warn "to run 'tally-cli' directly, add ~/.local/bin to PATH: export PATH=\"\$HOME/.local/bin:\$PATH\"" ;;
esac

# ── 4. Launch ────────────────────────────────────────────────────────────────
say "Launching Tally — look for it in the menu bar (top-right)…"
open "$APP" || warn "couldn't auto-launch; open it from $APP_DIR"

# ── 5. Offer: start at login (never forced) ──────────────────────────────────
if ask "Start Tally automatically at login?"; then
  if "$APP/Contents/MacOS/Tally" --register-login-item; then
    ok "login item set (if it says it needs approval, enable Tally in System Settings ▸ General ▸ Login Items)"
  else
    warn "could not register the login item — you can toggle it any time in Tally ▸ Settings"
  fi
else
  ok "skipped login item — toggle it any time in Tally ▸ Settings"
fi

# ── 6. Offer: Übersicht desktop widget (optional) ────────────────────────────
# Mirrors apps/ubersicht/install.sh's widget-deploy step, but verified from the
# pinned release and reusing the tally-cli just installed (no source build).
if ask "Also install the Übersicht desktop widget?"; then
  say "Downloading + verifying the widget…"
  fetch "tally.jsx"
  fetch "tally-usage.sh"
  verify "tally.jsx" "tally-usage.sh"
  mkdir -p "$WIDGET_DIR"
  cp -f "$TMP/tally.jsx"      "$WIDGET_DIR/tally.jsx"
  cp -f "$TMP/tally-usage.sh" "$WIDGET_DIR/tally-usage.sh"
  ...
  if [ -n "$UB" ]; then
    ok "Übersicht found — open it (or its menu ▸ Refresh All Widgets) to see Tally top-right"
  else
...
# ── 7. Summary + uninstall ───────────────────────────────────────────────────
printf '\n%sTally %s is installed.%s\n' "$BOLD" "$TAG" "$RST"
printf '  • App : %s\n' "$APP"
printf '  • CLI : %s/tally-cli   (try: tally-cli --json)\n' "$BIN_DIR"
[ -d "$WIDGET_DIR" ] && printf '  • Widget: %s\n' "$WIDGET_DIR"
printf '\nTo uninstall:\n'
printf '  "%s/Contents/MacOS/Tally" --unregister-login-item   # if you enabled login\n' "$APP"
printf '  rm -rf "%s"\n' "$APP"
printf '  rm -f  "%s/tally-cli"\n' "$BIN_DIR"
